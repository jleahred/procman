...
// use crate::types::config::ActiveProcessByConfig;

// use super::super::{ProcessStatus, RunningStatus};

// pub(crate) fn run_init_cmds(
//     mut rs: RunningStatus,
//     act_proc_by_cfg: &ActiveProcessByConfig,
// ) -> RunningStatus {
//     for (id, process) in rs.processes.iter_mut() {
//         if let Some(proc_cfg) = act_proc_by_cfg.0.get(&process.id) {
...
//                 match (act_proc_by_cfg.0.get(id), &proc_cfg.init_command) {
//                     (Some(_), Some(init_command)) => {
...
//                     (None, _) => {}
//                     (Some(_), None) => {
//                         println!("[{}] No init command provided for process", id.0);
//                         process.status = ProcessStatus::Running { pid };
//                     }
//                 }
//             }
