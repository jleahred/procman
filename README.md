...
An `id` can be repeated with different `apply_on`
...
`health_check` can receive a command with timeout

You can verify if a socket is operational, or if a lock file has been created... the command will depend on the context

Optionally, `health_check` can be given a directory to monitor for file activity
...
> DANGER! If the process takes a long time to start, and the `health_check` is queried before it's operational, the system will consider it as non-operational and mark the process as stopped. THIS COULD CAUSE DUPLICATE STARTS
