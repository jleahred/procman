...
`simple` for simple and direct commands. You cannot use bash operators as `&&`, `&`...

In legacy or simple systems, running a complex command can trick the `pid`

If the command is simple/direct, this strategy is very straightforward.

In case the command is complex and the system spawns multiple processes, depending on the system, some `kill` signals may not propagate among those processes, and `kill -9` never will.

If it is not possible or convenient to use the `simple` strategy, the service’s lifecycle can be managed with a `health_check` or a `pid helper`.
...
If true, This process configuration will be ignored
